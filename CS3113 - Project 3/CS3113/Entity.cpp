...
void Entity::checkCollisionY(Entity *collidableEntities, int collisionCheckCount)
{
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            if (collidableEntity->mEntityType == UFO) {
                mHitUFO = true;
            }
            
            // Check for platform landing from the top 
            if (collidableEntity->mEntityType == PLATFORM && mVelocity.y > 0) {
                mLandedOnPlatform = true;
            }
