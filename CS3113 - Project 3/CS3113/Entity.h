...
enum EntityType { PLAYER, UFO, PLATFORM, NONE }; // Inspired by in class exercise. This is used to differentiate between the different types of entities. 
...
private:
    ...
    bool mUseAcceleration = false; // Flag added since the player used acceleration based physics but the UFO and platform do not
...
public:
    ...
    bool hitUFO() const { return mHitUFO; }
    bool landedOnPlatform() const { return mLandedOnPlatform; }
    void resetGameFlags() { mHitUFO = false; mLandedOnPlatform = false; }
    ...
    void setAngle(float newAngle) 
        { mAngle = newAngle;                       }
    void UseAccelerationPhysics() 
        { mUseAcceleration = true; 
                                                   }
